...
│   ├── GoogleTagManager.tsx    # ★ контейнер GTM — все счётчики только отсюда
│   ├── CallNotifier.tsx        # ★ клик по tel: → «сейчас позвонят» в Telegram
...
scripts/
└── indexnow.mjs                # ★ npm run indexnow — сообщить поисковикам об обновлении
public/
└── <ключ>.txt                  # ⚠️ подтверждение владения для IndexNow, НЕ УДАЛЯТЬ
```

**После публикации новых страниц:** `npm run indexnow` — скрипт берёт адреса из живого
sitemap.xml и отправляет их в Bing, Yandex, Naver и Seznam. Индексация занимает часы
вместо недель. Для нас важен Bing: на его индексе работает поиск ChatGPT.
...
| `NEXT_PUBLIC_GA_ID` | GA4 `G-676GCTBX4Z`. ⚠️ Кодом **больше не используется** — счётчик настраивается тегом внутри GTM. Переменная осталась в Vercel как справка | не используется |
| `NEXT_PUBLIC_GOOGLE_ADS_ID` | Ads `AW-18365157406` (аккаунт **564-992-8278**). ⚠️ Кодом **больше не используется** — конверсия настраивается тегом внутри GTM | не используется |
| `NEXT_PUBLIC_GOOGLE_ADS_CALL_LABEL` | ярлык конверсии `A_YuCJzc7twcEJ6gmLVE`. ⚠️ Кодом **больше не используется** — задаётся в теге GTM | не используется |
| `NEXT_PUBLIC_GTM_ID` | Google Tag Manager, контейнер **GTM-KG9FC63K** — ★ единственное место, откуда ставятся счётчики. GA4 и Ads настраиваются тегами внутри контейнера, в коде их больше нет | ✅ добавлена в Vercel (3 среды), значение проверено |
...
**Про флаг «Sensitive» в Vercel.** Он означает ровно одно: значение **нельзя прочитать
обратно** после создания. На работу переменной не влияет — при сборке и в рантайме она
доступна как обычно (`TELEGRAM_BOT_TOKEN` помечен Sensitive и прекрасно работает).
Правило: настоящие секреты (токен бота, chat id) — Sensitive; всё с приставкой
`NEXT_PUBLIC_` — **Non-sensitive**, потому что оно и так попадает в исходный код
страницы, и Sensitive даёт ноль защиты, зато лишает возможности себя проверить
(`vercel env pull` в файл вне репозитория). Добавлять из CLI: `vercel env add ИМЯ
production --no-sensitive`. ⚠️ Sensitive нельзя включить для Development — только
Production и Preview.

⚠️ Любая `NEXT_PUBLIC_*` вшивается **в момент сборки**. Добавил переменную — нужен
новый билд: `vercel redeploy` берёт старую сборку и переменную не подхватит.
